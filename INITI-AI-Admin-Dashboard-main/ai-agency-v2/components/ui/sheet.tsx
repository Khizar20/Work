...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-gradient-to-br from-[hsla(279,34%,43%,0.95)] to-[hsla(248,79%,22%,0.95)] text-white p-6 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500",
  {
    variants: {
      side: {
...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <SheetPrimitive.Content
      ...
      {...props}
    >
      {children}      <SheetPrimitive.Close className="absolute right-4 top-4 rounded-full p-1 opacity-70 bg-white/10 transition-all hover:bg-white/20 hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-white/30 focus:ring-offset-1 disabled:pointer-events-none text-white">
        <X className="h-4 w-4" />
